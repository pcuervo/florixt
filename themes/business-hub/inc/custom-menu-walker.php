<?php
class businesshub_custom_walker_nav_menu extends Walker_Nav_Menu {
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth = 0, $args = array(),  $current_object_id = 0) {
		...
		$item_output = sprintf( '%1$s<a%2$s'.($is_parent&&$depth==0?' data-toggle="dropdown"':'').'>'.$icon_class_names.'%3$s%4$s%5$s' . ($is_parent&&$depth==0?'':'') .'</a>%6$s',
...
class businesshub_custom_walker_nav_menu_mobile extends Walker_Nav_Menu {
...
add_filter( 'wp_nav_menu_objects', 'businesshub_add_menu_parent_class' );
function businesshub_add_menu_parent_class( $items ) {
