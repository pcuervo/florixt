			...
			if(file_exists($this->_args['option_file'])){
				try {
					...
					foreach($sxe->tab as $tab){
						...
						$tab_titles .= '<li ' . ($tabidx == 1 ? 'class="uk-active"':'') .'><a href="#option-tab-'.$tabidx.'">'.$icon. esc_html__(''.$tab_name,'cactus').'</a></li>';
						...
						foreach($tab->group as $group){
							...
							$tab_contents .= 
								'<table class="uk-table" width="100%" cellpadding="0" cellspacing="0">';
									...
									$tab_contents .= '
									<thead>
										<th>'.$icon. esc_html__(''.$group_name,'cactus').'</th>
									</thead>';
									}
							...
							foreach($group->fields as $fields){
								...
								$tab_contents .=
									'<tr class="row label '.(count($fields->description) > 0?'no-border':'').'">
										<td align="left">
											<label>'
												. esc_html__(''.$fields_atts['label'],'cactus').'</label>
										</td>
									</tr>
									<tr class="row">
										<td>';
											foreach($fields->option as $option){
												...
												if(isset($atts['tooltip'])){
													$props = ' data-uk-tooltip title="' . esc_html__(''.$atts['tooltip'],'cactus') . '" ';
												}
												...
												if(isset($atts['label'])){
													$tab_contents .= '<label>' . esc_html__(''.$atts['label'],'cactus') . ':</label>';
												}
									...
									if(count($fields->description) > 0){
										foreach($fields->description as $desc){
											$tab_contents .= '<tr class="row description"><td>'.esc_html__(''.$desc,'cactus').'</td></tr>';
										}
									}
							}
					...
					echo esc_html__("Exception occurs: ",'cactus') . print_r($e);
				}
			}
