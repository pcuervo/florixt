 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class Custom_WP_Import extends WP_Importer {
	...
	function Custom_WP_Import() { /* nothing */ }
...
<?php
	 ...
	 */
	function process_posts($start = 1, $limit = 1000) {
		$counting = 0;
		...
		foreach ( $this->posts as $post ) {
			...
			if($counting > $start + $limit) break;
			if($counting < $start) continue;
	...
	function fetch_remote_file( $url, $post ) {
		...
		$headers = WP_Http::get( $url, array( $upload['file'] ) );
...
if(!function_exists('wordpress_importer_init')){
	function wordpress_importer_init() {
		 ...
		 */
		$GLOBALS['wp_import'] = new Custom_WP_Import();
